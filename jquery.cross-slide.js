        ...
        (function (proceed) {
            ...
            function computePlan(from, to) {
                ...
                if (opts.toggleDir) {
                    ...
                    switch (opts.toggleDir) {
                        case 'updown':
                            setDir(['up', 'down']);
                            break;
                    }

                    if (from && to && to === 0 && plan[from].dir === plan[to].dir) {
                        var d = (plan[to].dir === 'up') ? ['down', 'up'] : ['up', 'down'];
                        setDir(d);
                    }
                }
                ...
                for (var i = 0; i < plan.length; ++i) {
                    ...
                    if (speed) { // speed/dir mode
                        ...
                        if (slide_px <= 0) {
                            var ratio = p.img.width / p.img.height;
                            switch (p.dir) {
                                case 'up':
                                case 'down':
                                    p.height = p.img.height += -slide_px + fade_px;
                                    p.width = p.img.width = p.img.height * ratio;
                                    break;
                                case 'left':
                                case 'right':
                                    p.height = p.img.width += -slide_px + fade_px;
                                    p.width = p.img.height = p.img.width / ratio;
                                    break;
                            }
                            /*
            ...
            function create_chain(i, chainf) {
                ...
                if (i % 2 == 0) {
                    ...
                    if (i_to === 0 && opts.toggleDir) {
                        computePlan(i_from, i_to);
                    }
                    if (i_to > i_from) {
                        to_init.opacity = 0;
                        to_anim.opacity = 1;
